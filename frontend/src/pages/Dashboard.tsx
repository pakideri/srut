  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <KpiCard icon={Users} label="Total Candidates" value={data.totalCandidates} sub={`${data.activeCandidates} active`} color="bg-indigo-500" />
        <KpiCard icon={UserCheck} label="Hired" value={data.hired} color="bg-emerald-500" />
        <KpiCard icon={TrendingUp} label="Hiring Rate" value={`${data.hiringRate}%`} color="bg-violet-500" />
        <KpiCard
          icon={Clock}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="card">
          <h3 className="font-semibold text-gray-800 mb-4">Candidates by Stage</h3>
        ...
        <div className="card">
          <h3 className="font-semibold text-gray-800 mb-4">Candidates by Department</h3>
        ...
        <div className="card">
          <h3 className="font-semibold text-gray-800 mb-4">Recruiter Performance</h3>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="table-th pl-0">Recruiter</th>
                ...
                <tbody>
                  {data.recruiterPerf.map((r, i) => (
                    <tr key={i} className="border-b border-gray-50">
                      <td className="table-td pl-0 font-medium">{r.recruiter}</td>
                      <td className="table-td">{r.total}</td>
                      <td className="table-td">{r.hired}</td>
                      <td className="table-td">{r.total > 0 ? `${Math.round((r.hired / r.total) * 100)}%` : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
